    ...
    @push('scripts')
        ...
        const ALL_SUPPLIERS = @json($suppliers->map(function($supplier) {
            ...
            function handleProductChange(itemId, productId) {
                ...
                // Check for duplicate product (handle both numeric IDs and new names)
                const trackingValue = isNaN(parseInt(productId)) ? productId : parseInt(productId);
                if (addedProducts.has(trackingValue)) {
                ...
                addedProducts.add(trackingValue);
                ...
                // If it's a new product (string ID), show a small hint
