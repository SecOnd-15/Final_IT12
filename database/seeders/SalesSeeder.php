...
class SalesSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Generate sales for the last 3 months up to today
        $salesData = [];
        $saleItemsData = [];
        $paymentsData = [];
        ...
        // We'll generate data for the last 4 months (including current)
        $monthsToSeed = [
        ...
        foreach ($monthsToSeed as $monthDate) {
            ...
            if ($date->dayOfWeek === Carbon::SATURDAY) {
                ...
                for ($saleNum = 1; $saleNum <= $salesCount; $saleNum++) {
                    ...
                    $minute = rand(0, 59);
                    $saleDateTime = $date->copy()->hour($hour)->minute($minute)->second(rand(0, 59));
            ...
            // Show progress
            if ($day % 5 === 0) {
                $this->command->info("Generated sales for {$date->format('F d, Y')}...");
            }
        }
    }
    ...
    // Insert data in batches
        ...
        $this->command->info("✅ Successfully generated {$totalSales} sales!");
        $this->command->info("📅 Date range: " . $monthsToSeed[0]->format('M Y') . " to " . $now->format('M d, Y'));
